...
import { enviarCurriculo, validarConfiguracaoEmail } from '../email/email.service.js';
...
const carregarDadosCandidato = async () => {
  try {
    const candidateProfilePath = path.join(process.cwd(), 'candidate-profile.json');
...
const gerarCurriculoHTML = (candidato) => {
  const { personalInfo, experiences, education, certifications, skills, languages } = candidato;
...
export const enviarCurriculoTesteHTML = asyncHandler(async (req, res) => {
  ...
  const caminhoArquivoPdf = await gerarPdfCurriculo(curriculoPersonalizado, dadosVaga);
  
  // 5. Verificar configuração de e-mail
  if (!validarConfiguracaoEmail()) {
    logWarn('Configuração de e-mail não disponível, simulando envio', { requestId });
  ...
  try {
    ...
    if (config.server.env === 'development') {
      logWarn('Continuando sem envio de e-mail (modo desenvolvimento)', { requestId });
      resultadoEmail = {
        sucesso: false,
      ...
      throw new AppError(`Falha no envio do e-mail: ${emailError.message}`, 503);
    }
  }
  ...
  try {
    ...
    logInfo('Arquivo temporário removido', { arquivo: path.basename(caminhoArquivoPdf), requestId });
  } catch (error) {
    logWarn('Erro ao remover arquivo temporário', { error: error.message, requestId });
  }
  ...
  const response = {
    ...
    detalhes: {
      ...
      tecnologiasEncontradas: curriculoPersonalizado.matchingSkills?.length || 0,
