...
import { gerarResumo } from "./resumoProfissionalService.js";
...
const inferirSenioridade = (dadosVaga = {}) => {
  ...
  if (
    ["senior", "sênior", "staff", "especialista", "lead", "lider"]
      .some((termo) => textoVaga.includes(normalizeText(termo)))
  ) {
    return "senior";
  }

  if (["junior", "júnior", "estagio", "trainee"].some((termo) => textoVaga.includes(normalizeText(termo)))) {
    return "junior";
  }

  if (["pleno", "mid-level", "mid level"].some((termo) => textoVaga.includes(normalizeText(termo)))) {
    return "pleno";
  }
...
const calcularAnosExperiencia = (experiencias = []) => {
  ...
  experiencias.forEach((exp) => {
    ...
    const fim = exp.endDate === "present" ? agora : new Date(`${exp.endDate || ""}-01`);

    if (Number.isNaN(inicio.getTime()) || Number.isNaN(fim.getTime()) || fim <= inicio) {
      return;
    }

    const diffAnos = (fim.getTime() - inicio.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
...
const calcularPontuacaoRelevancia = (perfil, dadosVaga) => {
  ...
  const stackCoverage = calcularCoberturaLista(
    stackTecnologica,
    (tech) =>
